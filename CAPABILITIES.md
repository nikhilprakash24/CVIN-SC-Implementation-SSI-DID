...
> **Numbers in this file are descriptive, not the results of record.** The results of record are `4_comparison-framework/results/` (nine-standard gas, scaling, MOBI backends, V2V latency), `cv2x-testbed/results/` (PKI vs ERC-1056) and `docs/figures/results_snapshot.json`; every figure's condition and status is in `docs/MEASUREMENT_CONDITIONS.md`. Chapters cite those, never this file.
